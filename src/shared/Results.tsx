...
export function Results({ correct, total, mistakes = 0, failed = false, showModes = true, level, levelChange, rewardName, rewardIcon, rewardLost = false, collectionComplete = false, outcomeTitle, outcomeText, remainingHammers, firstTryRounds, onAgain, onModes, onHome }: ResultsProps) {
  ...
  return <main className="results-page"><motion.section className="results-card" initial={{ scale: .7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
    ...
    {firstTryRounds !== undefined && <div className="beaver-result-details"><span>Мосты с первой попытки <b>{firstTryRounds} из 10</b></span><span>Осталось молоточков <b>{remainingHammers ?? 0} из 5</b></span></div>}
    ...
    {collectionComplete ? <motion.div className="collection-complete" role="status" aria-label="Все награды собраны" initial={{ scale: .72, opacity: 0, rotate: -2 }} animate={{ scale: 1, opacity: 1, rotate: 0 }} transition={{ type: 'spring', stiffness: 150, damping: 13 }}>
      <div className="collection-sparkles" aria-hidden="true"><i>✦</i><i>★</i><i>✧</i><i>★</i><i>✦</i></div>
      <span className="collection-crown" aria-hidden="true">🏆</span>
      <div><b>Поздравляем! Все награды собраны!</b><strong>Ты — Хранитель лесной науки</strong><small>Полный комплект исследователя теперь в твоём рюкзаке.</small></div>
    </motion.div> : rewardName && <motion.div className={`reward-result ${rewardLost ? 'lost' : ''}`} initial={{ y: 12, opacity: 0 }} animate={{ y: 0, opacity: 1 }}><span>{rewardLost ? '🩹' : rewardIcon}</span><div><b>{rewardLost ? `${rewardName} потерян или сломан` : `Новая награда: ${rewardName}`}</b><small>{rewardLost ? 'Победи и получи предмет заново.' : 'Предмет добавлен в рюкзак исследователя.'}</small></div></motion.div>}
    <p>{failed ? 'Попытки не ограничены — можно сразу сыграть ещё раз.' : collectionComplete ? 'Это настоящее достижение! Новые победы продолжат повышать твой уровень.' : 'Уровень повышен. Продолжай лесное приключение!'}</p>
