...
import { explorerRewards, heartsForPercent } from '../shared/rewards'
...
export function App() {
  ...
  const finish = useCallback((correct: number, mistakes = 0, failed = false) => {
    ...
    const record: SessionRecord = { id: crypto.randomUUID(), gameId, gameTitle: game.title, modeId, modeTitle: mode.title, completedAt: new Date().toISOString(), correct, total, percent, hearts, mistakes, outcome: failed ? 'loss' : 'win' }
  ...
  return <>
    ...
    {screen === 'profile' && <Profile profile={profile} onBack={() => setScreen('home')} onRename={() => setModal('rename')} onReset={() => { if (confirm('Удалить имя, историю, уровни, предметы и все сердца?')) { clearProfile(); setProfile(null); setScreen('home') } }}/>}
    {screen === 'modes' && <ModeSelect game={game} onBack={() => setScreen('home')} onSelect={id => { setModeId(id); setScreen('game'); audio.play('tap') }}/>} 
    {screen === 'game' && (gameId === 'frog' ? <FrogGame {...gameProps} onFail={(score, mistakes) => finish(score, mistakes, true)}/> : gameId === 'squirrel' ? <SquirrelGame {...gameProps}/> : <BeaverGame {...gameProps}/>)}
