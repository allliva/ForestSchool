...
describe('игра бобра', () => {
  ...
  it('поддерживает клавиатурно доступные кнопки слов и пять молоточков', () => {
    render(<BeaverGame modeId="order" onFinish={vi.fn()} onFail={vi.fn()} onExit={vi.fn()} onHelp={vi.fn()} onAudio={vi.fn()}/>)
    expect(screen.getByLabelText('Осталось молоточков: 5').querySelectorAll('img.active')).toHaveLength(5)
  ...
  it('засчитывает правильный мост и сразу увеличивает прогресс', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('Путь домой: 1 из 10')).toBeTruthy(), { timeout: 3000 })
  })

  it('после пятого неверного моста завершает сессию поражением', async () => {
    const onFail = vi.fn()
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      if (attempt < 4) {
        await waitFor(() => {
          const nextLogs = screen.getByRole('group', { name: 'Доступные брёвна' }).querySelectorAll('button')
          expect(nextLogs.length).toBeGreaterThan(0)
        }, { timeout: 4500 })
      }
    }
    await waitFor(() => expect(onFail).toHaveBeenCalledWith(0, 5))
  })
})
